/**
 * QtJson - A simple class for parsing JSON data into a QVariant hierarchies and
 * vice-versa. Copyright (C) 2011  Eeli Reilin
 *
...
QByteArray serializeMap(const T & map, bool & success) {
    ...
    for (typename T::const_iterator it = map.begin(), itend = map.end();
         it != itend; ++it) {
...
QByteArray serialize(const QVariant & data, bool & success) {
    ...
    if (!data.isValid()) {  // invalid or null?
        ...
        str = QByteArray::number(data.value<qlonglong>());
                                           ...
                                           // because all smaller types can be
                                           // converted to qlonglong
        ...
        str = sanitizeString(dateTimeFormat.isEmpty()
                                 ...
                                 : data.toDateTime().toString(dateTimeFormat))
                  .toUtf8();
    } else if (data.type() == QVariant::Date) {  // date value?
        str = sanitizeString(dateTimeFormat.isEmpty()
                                 ? data.toDate().toString()
                                 : data.toDate().toString(dateFormat))
                  .toUtf8();
    } else if (data.canConvert<QString>()) {  // can value be converted to
                                              // string?
        // this will catch QUrl, ... (all other types which can be converted to
        // string)
        str = sanitizeString(data.toString()).toUtf8();
    } else {
        success = false;
